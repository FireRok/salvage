// last-good/fleet capabilities for the borg engine (spec 0029): the engine
// implements spi.ChainTester and spi.FleetSurveyor, the near-exact sibling of
// the restic capabilities (the same relationship spec 0022 established for
// Restore). Chain/Survey read metadata from one `borg list --json` call inside
// the idle container; TestBackup extracts the named archive — borg has no
// "latest" alias, so every candidate is an explicit archive name, which is
// exactly what a chain walk needs. Each TestBackup is a real extract into a
// throwaway container; `last-good -max` is the bound for long histories
// (spec 0029 R6).
package borg
...
// listCmd lists the repository's archives as JSON. stderr is merged so a
// failure's reason (bad passphrase, missing repo) survives into the captured
// output; parseArchives skips any non-JSON preamble.
const listCmd = "borg list --json 2>&1"

// Chain enumerates the repository's archives newest-first, standing up a
// short-lived idle container just to read `borg list --json`. Part of
// spi.ChainTester (spec 0029 R3).
func (e Engine) Chain(ctx context.Context, cfg *config.Config) ([]spi.Backup, error) {
	...
	if exit != 0 {
		return nil, fmt.Errorf("borg list: %s", commandFailReason(out, exit))
	}
	return parseArchives(out)
}

// TestBackup extract-tests one archive (pinned by its name) with cfg's checks
// in a fresh throwaway container — the existing extract path (including the
// two-phase network isolation inside ephemeral's Restore) pinned to label
// instead of the configured archive, then the configured checks. Returns "" on
// success or a short failure reason. Part of spi.ChainTester (spec 0029 R3).
func (Engine) TestBackup(ctx context.Context, cfg *config.Config, label string) string {
	...
	env, err := ephemeral.StartBorg(tctx, cfg.Target.Restore.Image,
...
// extract (spec 0011 R3, spec 0029 R4). An unreachable or failing repository is
...
// source unchanged, repository, archive pin, and credentials carried over — a
// single-repo engine has no per-stanza swap (spec 0029 R4). The emitted
// skeleton is a directly usable `run` input.
...
// metadata stands up the idle container, runs the one archive-listing call, and
// tears the container down. err is operational only (Docker unavailable, a
// missing pass_env secret, exec failure); a failing borg command comes back as
// (output, non-zero exit, nil).
func (Engine) metadata(ctx context.Context, cfg *config.Config) (string, int, error) {
	...
	defer env.Stop()
	return env.RunCommand(ctx, listCmd)
}
...
// repository forwarded by name via pass_env (BORG_REPO) may be a secret and is
// never echoed — which is also why `borg list --json`'s repository.location
...
// merged output — borg prefixes/phrases real causes recognizably — falling back
// to the first non-empty line, then the bare exit code.
